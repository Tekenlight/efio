...
extern ev_piqueue_type create_ev_mpiqueue(int n);
extern void enqueue_ev_mpiqueue(ev_piqueue_type pq_ptr,void * data);
...
void * thr(void *param)
{
	...
	for (i=0;i<COUNT;i++,j++) {
		...
		enqueue_ev_mpiqueue(pq_ptr,(void*)(long)j);
		//enqueue_ev_piqueue(pq_ptr,(void*)(long)j);
		//usleep(1);
		if (NULL != dequeue_ev_mpiqueue(pq_ptr)) inp->dequeued++;
...
static void test_main(CuTest *tc)
{
	...
	*/
	...
	{
		void * p;
		while (NULL != (p = dequeue_ev_mpiqueue(q))) {
			++i;
			fd = (int)(long)p;
			printf("[MAIN]:%d:%d\n",i,fd);//fflush(stdout);
		}
