...
static void * thread_loop(void *data)
{
	...
	for (;;) {
		...
		if (s) break;
		qe = dequeue_evpaq(pool->_task_queue);
		if (!qe) {
			int i = 0;
			while (s == 0) {
				if (i<20) i++;
...
struct thread_pool_s * create_thread_pool(int num_threads)
{
	...
	pool->_num_threads = 0;
	pool->_cond_count = 0;
	pool->_task_queue = create_evpaq(4);
	pool->_shutdown = 0;
	pool->_enq_index = 0;
	pool->_free_thr_queue = create_evpaq(4);
	{
		sigset_t				set;
		sigset_t				o_set;
		int						sig = 0;
...
void free_thread_pool(struct thread_pool_s *pool)
{
    ...
    free(pool->_threads);
	destroy_evpaq(&(pool->_task_queue));
	destroy_evpaq(&(pool->_free_thr_queue));
...
static void wake_all_threads(struct thread_pool_s *pool, int immediate)
{
	...
	for (int i = 0; i < pool->_num_threads; i++) {
		...
		enqueue_evpaq(pool->_task_queue,qe);
	}
	return;
}
